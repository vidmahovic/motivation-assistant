...
*/
	...
	//	Until you edit this function, don't show login, register or logout links
		...
		return array(
			'login' => 'http://'.$_SERVER['HTTP_HOST'].'/auth/login',
...
*/
	...
	//	Until you edit this function, nobody is ever logged in
    	...
    	if($r["is_loggedin"]){
	    	...
	    	return array(
			    'userid' => $r['userid'],
			    'publicusername' => $r['ime'],
			    'email' => $r['email'],
			    'level' => (intval($r['userid']) == 2) ? QA_USER_LEVEL_ADMIN : QA_USER_LEVEL_BASIC
	    	);
...
/*        session_start();
        ...
        if ($_SESSION['is_logged_in']) {
            ...
            $result = mysql_fetch_assoc(
                mysql_query(
                    "SELECT email, ime, priimek FROM oseba WHERE userid=".mysql_real_escape_string($userid, $qa_db_connection).";",
                    $qa_db_connection
                )
            );
            ...
            if (is_array($result))
                return array(
                    'userid' => $userid,
                    'publicusername' => $_SESSION['public_username'],//$result['ime']." ".$result['priimek'],
                    'email' => $_SESSION['public_username'], //$result['email'],
                    'level' => ($userid==2) ? QA_USER_LEVEL_ADMIN : QA_USER_LEVEL_BASIC
                );
        }
*/
		...
		if ($_SESSION['is_logged_in']) {
			...
			if (is_array($result))
				return array(
					...
					'email' => $result['email'],
					'level' => QA_USER_LEVEL_BASIC	//'level' => ($userid=='admin') ? QA_USER_LEVEL_ADMIN : QA_USER_LEVEL_BASI
				);
		}
...
*/
	...
	//	Until you edit this function, always return null
		...
		if(isset($_SESSION['public_username'])){
			$email = $_SESSION['public_username'];
			return $email;
		}
		return null;
	...
	//	Until you edit this function, always return null
        ...
        if (count($publicusernames)) {
            ...
            $results=mysqli_query(
                $qa_db_connection,
                'SELECT email, userid FROM oseba WHERE email IN ('.implode(',', $escapedusernames).')'
                
            );

            while ($result=mysqli_fetch_assoc($results))
                $publictouserid[$result['email']]=$result['userid'];
        }
...
*/
	{
		...
		if (count($userids)) {
			...
			$results=mysqli_query(
				$qa_db_connection,
				'SELECT ime, priimek, userid FROM oseba WHERE userid IN ('.implode(',', $escapeduserids).')'
				
			);

			while ($result=mysqli_fetch_assoc($results)) {

				//$username = ucfirst($result['ime'])." ".ucfirst($result['priimek']);
				$useridtopublic[$result['userid']]= ucfirst($result['ime'])." ".ucfirst($result['priimek']);
				//$useridtopublic[$result['userid']] = ucfirst($result['ime'])." ".ucfirst($result['priimek']);
			}
		}
...
/*
	...
	qa_get_logged_in_user_html($logged_in_user, $relative_url_prefix)		//NE RABIS!!!
...
*/
	...
	//	By default, show the public username linked to the Q2A profile page for the user
		...
		return '<a href="'.htmlspecialchars($relative_url_prefix.'../users/'.urlencode($userid)).
...
/*
	...
	qa_get_users_html($userids, $should_include_link, $relative_url_prefix)		//NE RABIS!!!
	...
	//	By default, show the public username linked to the Q2A profile page for each user
		...
		foreach ($userids as $userid) {
			...
			if ($should_include_link)
				$usershtml[$userid]='<a target="_parent" href="'.htmlspecialchars($relative_url_prefix.'../users/'.urlencode($userid)).
		...
		foreach ($userids as $userid) {
			...
			if ($should_include_link)
				$usershtml[$userid]='<a href="'.qa_path_html('users/'.$publicusername).'" class="qa-user-link">'.$usershtml[$userid].'</a>';
		}
...
/*
	...
	qa_avatar_html_from_userid($userid, $size, $padding)		//NE RABIS!!!
...
/*
	...
	qa_user_report_action($userid, $action)			//NE RABIS!!!
