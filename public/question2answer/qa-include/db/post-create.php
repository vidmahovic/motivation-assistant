...
/*	{
		...
		$postid = qa_db_last_insert_id();

		if (str_contains($type, 'Q')) {
			$qa = $postid;
			$qa_1 = str_slug($title);
		} else if (str_contains($type, 'A')) {
			$qa = $parentid;
			$query = qa_db_query_sub('SELECT title FROM ^posts WHERE postid=$', $parentid);
			$reult = $query->fetch_array(MYSQLI_ASSOC);
			$qa_1 = str_slug($result['title']);
			Event::fire(new App\Events\AnswerWasCreated($userid, $postid));
		} else {
			$query = qa_db_query_sub('SELECT a.postid, a.title FROM ^posts a, ^posts b WHERE a.postid=b.parentid AND b.postid=$', $parentid);
		...
		$params = [
			...
			'notify'   => $notify,
			'url'	   => $_SERVER['PHP_SELF'].'?qa='.strval($qa).'&qa_1='.strval($qa_1)
		];
		if($type == 'A') {
			Event::fire(new App\Events\AnswerWasCreated($userid, $postid));
		}
	...
	}*/
	{
		...
		$id = qa_db_last_insert_id();

		if($type == 'Q') {
			Event::fire(new \App\Events\QuestionWasCreated($userid, $id));
		} else if($type == 'A') {
			Event::fire(new \App\Events\AnswerWasCreated($userid, $id));
		}
